...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
determine_projection <- function(sf_obj, proj_type = c("ortho", "aeqd", "albers", "longlat"), quiet = quiet) {
  with_error_handling(
    {
      bbox <- st_bbox(sf_obj)
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
get_noaa_relief <- function(lon1, lon2, lat1, lat2, data_dir, out_dir, arc_seconds = 30, res = NULL, add_geoid = FALSE, sea_level_offset = NULL, quiet = FALSE) {
  with_error_handling({
    ...
    if (!file.exists(base_relief_file)) {
      if (!quiet) cat(" .. Downloading ETOPO2022 global relief model (", arc_seconds, " arc-second)\n", sep = "")
      old_timeout <- getOption("timeout")
      options(timeout = max(2400, old_timeout))
      download.file(base_relief_url, base_relief_file, mode = "wb", quiet = quiet)
      options(timeout = old_timeout)
    }

    base_geoid_file <- file.path(data_dir, basename(base_geoid_url))
    if (!file.exists(base_geoid_file)) {
      if (!quiet) cat(" .. Downloading ETOPO2022 global geoid model (", arc_seconds, " arc-second)\n", sep = "")
      old_timeout <- getOption("timeout")
      options(timeout = max(2400, old_timeout))
      download.file(base_geoid_url, base_geoid_file, mode = "wb", quiet = quiet)
      options(timeout = old_timeout)
    }
