...
#######################################################
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
krige_submap_transect_set <- function(sf_hf,
                                      data_col,
                                      fitted_variogram,
                                      sf_grid,
                                      max_n_point_pairs = 10,
                                      max_point_pair_distance = 1e5,
                                      quiet = FALSE) {
  with_error_handling(
    {
      if (!quiet) cat(" .. Kriging: interpolating ", nrow(sf_hf), " observations onto ", length(sf_grid), " grid points\n", sep = "")
      proj <- determine_projection(sf_obj = sf_hf, proj_type = "aeqd", quiet = quiet)
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
make_experimental_vgrm <- function(sf_hf, data_col, variogram_cutoff, n_variogram_lags, quiet = FALSE) {
  with_error_handling({
    bbox <- st_bbox(sf_hf)
    ...
    with_error_handling(
      variogram(reformulate("1", response = data_col), locations = sf_hf, cutoff = lag_cutoff, width = bin_width),
      default = NULL, quiet = quiet
    )
  })
}

# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
fit_variogram_model <- function(experimental_variogram, variogram_model, quiet = FALSE) {
  with_error_handling(
    {
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
krige_cross_validation <- function(sf_hf, data_col, fitted_variogram, max_n_point_pairs, max_point_pair_distance, fold_vector, quiet = FALSE) {
  with_error_handling({
    with_error_handling(
      {
        krige.cv(
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
log_data <- function(path, data, quiet = FALSE) {
  with_error_handling(
    {
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
sample_and_shuffle_equal_nfolds <- function(n, n_folds = 10, seed = 42, quiet = FALSE) {
  with_error_handling(
    {
      set.seed(seed)
      fold_size <- n %/% n_folds
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
cost_function <- function(sf_hf, out_dir,
                          variogram_cutoff = 1,
                          n_variogram_lags = 30,
                          max_n_point_pairs = 10,
  ...
  with_error_handling(
    {
      ...
      experimental_variogram <- make_experimental_vgrm(
        sf_hf = sf_hf, data_col = "ihfc2024_obs", variogram_cutoff = variogram_cutoff, n_variogram_lags = n_variogram_lags, quiet = quiet
      )
      if (is.null(experimental_variogram)) {
        return(list(penalty_cost, penalty_cost, penalty_cost))
      }

      fitted_variogram <- fit_variogram_model(experimental_variogram = experimental_variogram, variogram_model = variogram_model, quiet = quiet)
      if (is.null(fitted_variogram)) {
        return(list(penalty_cost, penalty_cost, penalty_cost))
      }

      krige_cv <- krige_cross_validation(
        sf_hf = sf_hf, data_col = "ihfc2024_obs", fitted_variogram = fitted_variogram, max_n_point_pairs = max_n_point_pairs,
        max_point_pair_distance = max_point_pair_distance, fold_vector = fold_vector, quiet = quiet
      )
      if (is.null(krige_cv)) {
        return(list(penalty_cost, penalty_cost, penalty_cost))
      }
      ...
      log_data(path = out_path, data = data, quiet = quiet)
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
translate_nlopt_results_to_variogram_model <- function(sf_hf, data_col, variogram_model, nlopt_object, quiet = FALSE) {
  with_error_handling(
    {
      suppressWarnings({
        experimental_variogram <- make_experimental_vgrm(
          sf_hf = sf_hf, data_col = data_col, variogram_cutoff = nlopt_object$solution[1], n_variogram_lags = nlopt_object$solution[2], quiet = quiet
        )
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
optimize_krige_model <- function(data_dir,
                                 out_dir,
                                 submap_transect_set,
                                 variogram_model,
                                 n_starts = 100,
                                 ...
                                 variogram_weight = 0.5,
                                 na_threshold_proportion = 0.5,
                                 initial_guess = c(3, 30, 10, 20),
                                 lower_bounds = c(3, 20, 2, 10),
                                 upper_bounds = c(12, 50, 50, 50),
                                 nlopt_algorithm_global = "NLOPT_GN_MLSL_LDS",
                                 max_nlopt_evaluations_global = 100,
                                 x_tolerance_global = 1e-4,
                                 f_tolerance_global = 1e-4,
                                 nlopt_algorithm_local = "NLOPT_LN_NELDERMEAD",
                                 max_nlopt_evaluations_local = 500,
                                 x_tolerance_local = 1e-5,
                                 f_tolerance_local = 1e-5,
                                 nlopt_algorithm_fine_tune = "NLOPT_LN_NELDERMEAD",
                                 max_nlopt_evaluations_fine_tune = 500,
                                 x_tolerance_fine_tune = 1e-8,
                                 f_tolerance_fine_tune = 1e-8,
                                 seed = 42,
                                 quiet = FALSE) {
  with_error_handling({
    dirs <- list(
    ...
    if (file.exists(nlopt_path) && file.exists(nlopt_iter_path_run)) {
      if (!quiet) cat(" -- Found nlopt data: ", nlopt_path, "\n", sep = "")
      return(invisible())
    }
    ...
    proj <- determine_projection(sf_obj = data$hull, proj_type = "aeqd", quiet = quiet)
    sf_hf <- st_transform(data$ihfc2024_obs_hull[[1]], proj$wkt)
    fold_vector <- sample_and_shuffle_equal_nfolds(n = nrow(sf_hf), seed = seed, n_folds = n_folds, quiet = quiet)
    ...
    prev_x <- NULL
    prev_f <- NULL
    exploration <- explore_parameter_space(
      n_starts = n_starts, n_top = n_top, lower = lower_bounds, upper = upper_bounds, cost_fun = nlopt_fun, seed = seed, quiet = quiet
    )
    ...
    if (is.null(fine_tune_opt) || fine_tune_opt$status < 0 || fine_tune_opt$iterations < 10) {
      if (!is.null(fine_tune_opt)) {
        ...
        log_data(path = nlopt_iter_path_fail, data = data, quiet = quiet)
      } else {
        data <- list(
          nlopt_status = NULL,
          nlopt_iterations = fine_tune_opt$iterations,
      ...
      nlopt_out <- translate_nlopt_results_to_variogram_model(
        sf_hf = sf_hf, data_col = data_col, variogram_model = variogram_model, nlopt_object = fine_tune_opt, quiet = quiet
      )
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
explore_parameter_space <- function(n_starts, n_top, lower, upper, cost_fun, seed = 42, quiet = FALSE) {
  with_error_handling(
    {
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
process_submap_transect_sets <- function(data_dir,
                                         out_dir,
                                         submap_transect_sets,
                                         variogram_models = c("Sph", "Exp", "Gau"),
                                         seed = 42,
                                         nprocs = NULL,
                                         parallel = TRUE,
                                         quiet = FALSE) {
  ...
  with_error_handling({
    ...
    if (parallel) {
      ...
      future_walk2(
        data$set,
        data$model,
        ~ optimize_krige_model(data_dir = data_dir, out_dir = out_dir, .x, .y, quiet = quiet),
        .options = furrr_options(seed = seed)
      )
    } else {
      walk2(data$set, data$model, ~ optimize_krige_model(data_dir = data_dir, out_dir = out_dir, .x, .y, quiet = quiet))
    }
  })
}
